...
| 文件 | 说明 | 重要性 |
|------|------|--------|
| `src/WtCtaStraFact/WtStraDualThrust.cpp` | DualThrust策略实现（最佳学习示例） | ⭐⭐⭐ |
| `src/WtCtaStraFact/WtStraDualThrust.h` | DualThrust策略头文件 | ⭐⭐⭐ |
| `src/Includes/CtaStrategyDefs.h` | CTA策略基类定义 | ⭐⭐⭐ |
| `src/Includes/ICtaStraCtx.h` | 策略上下文接口（策略可用的所有接口） | ⭐⭐⭐ |

### ⭐⭐⭐ 引擎核心（必读）

| 文件 | 说明 | 重要性 |
|------|------|--------|
| `src/WtCore/WtCtaEngine.h` | CTA引擎接口定义 | ⭐⭐⭐ |
| `src/WtCore/WtCtaEngine.cpp` | CTA引擎实现（事件驱动、策略调度） | ⭐⭐⭐ |
| `src/WtCore/WtEngine.h` | 引擎基类（持仓、资金、数据管理） | ⭐⭐⭐ |
| `src/WtCore/WtEngine.cpp` | 引擎基类实现 | ⭐⭐⭐ |
| `src/WtCore/CtaStraContext.h` | 策略上下文头文件 | ⭐⭐⭐ |
| `src/WtCore/CtaStraContext.cpp` | 策略上下文实现（下单、查询接口） | ⭐⭐⭐ |
...
| `src/WtRunner/WtRunner.cpp` | 主程序实现（初始化流程） | ⭐⭐ |
| `dist/WtRunner/config.yaml` | 运行配置文件 | ⭐⭐ |
...
| `src/WtCore/ParserAdapter.cpp` | 行情适配器实现 | ⭐⭐ |
| `src/WtCore/TraderAdapter.h` | 交易接口适配器 | ⭐⭐ |
| `src/WtCore/TraderAdapter.cpp` | 交易适配器实现 | ⭐⭐ |
| `src/WtCore/WtDtMgr.h` | 数据管理器 | ⭐⭐ |
| `src/QuoteFactory/main.cpp` | QuoteFactory主程序 | ⭐⭐ |

### ⭐ 接口实现（参考）

| 文件 | 说明 | 重要性 |
|------|------|--------|
| `src/ParserXTP/ParserXTP.cpp` | XTP行情解析器实现 | ⭐ |
| `src/TraderXTP/TraderXTP.cpp` | XTP交易接口实现 | ⭐ |
...
2. `src/WtCore/WtCtaEngine.h` - CTA引擎接口
...
2. `src/Includes/ICtaStraCtx.h` - 策略上下文接口
...
1. **创建策略类**
   - 参考: `src/WtCtaStraFact/WtStraDualThrust.h/cpp`
   - 继承: `CtaStrategy`
   - 实现: `CtaStrategyDefs.h` 中定义的接口

2. **注册到工厂**
   - 文件: `src/WtCtaStraFact/WtCtaStraFact.cpp`
   - 参考: `createStrategy()` 函数

3. **配置策略**
   - 文件: `dist/WtRunner/config.yaml`
   - 位置: `strategies.cta` 配置项

### 如何获取K线数据？

**代码位置**: `src/WtCtaStraFact/WtStraDualThrust.cpp:77`

```cpp
WTSKlineSlice *kline = ctx->stra_get_bars(code.c_str(), _period.c_str(), _count, true);
```

**接口定义**: `src/Includes/ICtaStraCtx.h:116`

### 如何下单？

**代码位置**: `src/WtCtaStraFact/WtStraDualThrust.cpp` (查找 `stra_enter_long`)

**接口定义**: `src/Includes/ICtaStraCtx.h:65-68`

**实现位置**: `src/WtCore/CtaStraContext.cpp` (查找 `stra_enter_long`)

### 如何查询持仓？

**代码位置**: `src/WtCtaStraFact/WtStraDualThrust.cpp:60`

```cpp
double pos = ctx->stra_get_position(code.c_str());
```

**接口定义**: `src/Includes/ICtaStraCtx.h:76`

### 如何处理Tick数据？

**代码位置**: `src/WtCtaStraFact/WtStraDualThrust.cpp` (查找 `on_tick`)

**接口定义**: `src/Includes/CtaStrategyDefs.h:78`

**调用链**: `src/WtCore/WtCtaEngine.cpp::on_tick()`

### 如何处理K线闭合？

**代码位置**: `src/WtCtaStraFact/WtStraDualThrust.cpp` (查找 `on_bar`)

**接口定义**: `src/Includes/CtaStrategyDefs.h:83`

**调用链**: `src/WtCore/WtCtaEngine.cpp::on_bar()`
...
| 配置文件 | 说明 |
|---------|------|
| `dist/WtRunner/config.yaml` | WtRunner主配置 |
| `dist/WtRunner/tdparsers.yaml` | 行情解析器配置 |
| `dist/WtRunner/tdtraders.yaml` | 交易接口配置 |
| `dist/QuoteFactory/dtcfg.yaml` | QuoteFactory配置 |
| `dist/QuoteFactory/mdparsers.yaml` | 行情解析器配置 |
| `dist/common/stocks.json` | 股票合约列表 |
| `dist/common/contracts.json` | 期货合约列表 |
| `dist/common/sessions.json` | 交易时间模板 |
...
**快速开始**: 先阅读 `src/WtCtaStraFact/WtStraDualThrust.cpp`，这是最好的学习起点！
