...
bool yaml_to_variant(const YAML::Node &root, WTSVariant *params) {
  ...
  for (auto &m : root) {
    ...
    case YAML::NodeType::Scalar:
      // 检查是否为布尔值
      try {
        bool boolVal = item.as<bool>();
        if (isMap)
          params->append(key.c_str(), boolVal);
        else
          params->append(boolVal);
      } catch (...) {
        // 不是布尔值，按字符串处理
        if (isMap)
...
WTSVariant *WTSCfgLoader::load_from_content(const std::string &content,
                                            bool isYaml /* = false */) {
  // 加一个自动检测编码的逻辑
  bool isUTF8 =
      EncodingHelper::isUtf8((unsigned char *)content.data(), content.size());
